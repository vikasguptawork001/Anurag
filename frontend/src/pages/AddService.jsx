  ...
  return (
    <div className="mx-auto max-w-5xl pb-2">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Workshop</p>
          <h1 className="mt-0.5 text-xl font-bold text-slate-900 sm:text-2xl">Add service record</h1>
          <p className="mt-1 text-sm text-slate-600">Capture job card details and due-date logic in one pass.</p>
        </div>
      </div>
      ...
      >
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 sm:px-6">
          <h2 className="text-sm font-semibold text-slate-900">Vehicle &amp; job</h2>
          <p className="text-xs text-slate-500">Identify the vehicle, then enter the job card.</p>
        </div>
        ...
        <div className="space-y-5 p-4 sm:p-6">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
            <div className="lg:col-span-7">
              ...
              <input
                ...
                className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 uppercase"
                placeholder="e.g. MH12AB1234"
                autoComplete="off"
              />
              {vehicleNumber.trim() && filteredVehicles.length > 0 && (
                <ul className="mt-2 max-h-32 overflow-auto rounded-lg border border-slate-200 bg-slate-50 text-sm scrollbar-thin">
                  {filteredVehicles.map((v) => (
                    <li key={v.id}>
                      ...
                      >
                        <span className="font-medium text-slate-900">{v.vehicle_number}</span>
                        <span className="text-xs text-slate-600">{v.owner_name}</span>
                      </button>
                    </li>
                  ))}
                </ul>
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50/80 p-3 lg:col-span-5">
              <p className="text-xs font-semibold text-slate-600">Estimated next due date</p>
              <p className="mt-1 text-[11px] text-slate-500">From odometer, next due KM, avg km.</p>
          ...
          {showNewVehicle && (
            <div className="rounded-lg border border-amber-200 bg-amber-50/80 p-4 sm:p-5">
              <p className="text-sm font-semibold text-amber-900">New vehicle</p>
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div>
                  <label className="text-xs font-medium text-slate-700">Avg daily KM *</label>
                  <input
                    type="number"
                    min={0.1}
                    step={0.1}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="text-sm font-medium text-slate-700">Job card no. *</label>
              <input
                value={jobCardNo}
            ...
            <div>
              ...
              <div className="mt-2.5 flex h-11 items-center gap-6 rounded-lg border border-slate-200 bg-slate-50 px-3">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div>
              <label className="text-sm font-medium text-slate-700">Work done</label>
              <textarea
                value={workDone}
            ...
            <div>
              <label className="text-sm font-medium text-slate-700">Parts replaced</label>
              <textarea
                value={partsReplaced}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="text-sm font-medium text-slate-700">Next due (KM) *</label>
              <input
                type="number"
                value={nextDueKm}
            ...
            <div>
              <label className="text-sm font-medium text-slate-700">Avg daily KM</label>
              <input
                type="number"
                min={0.1}
                step={0.1}
              ...
              {vehicleId && !showNewVehicle ? (
                <p className="mt-1 text-[11px] text-slate-500">Updates vehicle if changed.</p>
              ) : null}
            </div>
            <div className="sm:col-span-2">
          ...
          <div>
            <span className="text-sm font-medium text-slate-700">Follow-up call done?</span>
        ...
        <div className="sticky bottom-0 z-20 flex flex-col-reverse gap-2 border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur sm:flex-row sm:items-center sm:justify-end sm:gap-3 sm:px-6">
          <p className="mr-auto hidden text-xs text-slate-500 sm:block">All required fields must be filled.</p>
          <button
            type="submit"
            disabled={submitting}
